...
export default function SupportingDocuments() {
  ...
  return (
    <div className="min-h-screen bg-[#010308] text-white flex flex-col relative">
      ...
      {/* Main Content */}
      <main className="flex-grow mx-auto max-w-6xl w-full px-6 py-10 space-y-8 relative z-10 text-left">
        
        {/* Title */}
        <div>
          <h1 className="text-2xl font-extrabold text-white">Supporting Financial Documents</h1>
          <p className="text-white/40 text-xs mt-0.5">Upload utility receipts to verify your payment cycles and automatically improve your score</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="glass-card p-6 rounded-xl border-white/10 space-y-5">
              <div>
                <h3 className="font-bold text-white text-sm">Upload Billing Document</h3>
                <p className="text-white/40 text-[10px] leading-relaxed mt-1">
                  We process Electricity, Water, Internet, Mobile, Gas, Rent receipts, and Bank Statements. Accepted formats: PDF, PNG, JPG, JPEG (Max 5MB).
                </p>
              </div>

              {uploadError && (
                <div className="p-3.5 rounded bg-[#D1495B]/10 border border-[#D1495B]/20 text-[#D1495B] text-xs flex gap-2">
              ...
              {uploadSuccess && (
                <div className="p-3.5 rounded bg-[#34C759]/10 border border-[#34C759]/20 text-[#34C759] text-xs flex gap-2">
              ...
              <form onSubmit={handleUploadSubmit} className="space-y-4">
                <div className="border border-dashed border-white/15 hover:border-[#59CFFF]/50 rounded-lg p-6 text-center cursor-pointer transition-colors relative bg-[#030E21]/20">
                  <input
                    type="file"
                ...
                {selectedFile && (
                  <div className="bg-[#030E21]/30 p-3 rounded border border-white/5 flex items-center justify-between text-xs font-mono">
                    <span className="truncate text-white/60">{selectedFile.name}</span>
                    <span className="text-white/30">({Math.round(selectedFile.size / 1024)} KB)</span>
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full py-3 rounded-lg btn-glow-sky text-xs font-bold flex justify-center items-center gap-1.5 disabled:opacity-50"
                >
                  {uploadLoading ? (
                    <>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="glass-card p-6 rounded-xl border-white/10 space-y-4">
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                <div>
                  <h3 className="font-bold text-white text-sm">Document History</h3>
                  <p className="text-[10px] text-white/40 mt-0.5">Verified receipts mapping to credit scoring calculation</p>
                </div>
                <button
                  onClick={fetchBills}
                  className="p-2 rounded hover:bg-white/5 text-white/50 hover:text-white transition-all"
              ...
              {loading ? (
                ...
                <div className="text-center py-12 text-xs text-[#D1495B]">
                  {error}
                </div>
              ) : bills.length === 0 ? (
                <div className="text-center py-16 text-xs text-white/30 space-y-2.5">
                  <FileText className="h-8 w-8 mx-auto opacity-20" />
                  <p>No supporting documents uploaded yet.</p>
                  <p className="text-[9px] text-white/20">Upload bills above to compute your payment consistency score.</p>
                </div>
              ) : (
                <div className="overflow-x-auto rounded-lg border border-white/5 bg-[#030E21]/15">
                  <table className="w-full text-xs text-left border-collapse">
                    <thead>
                      <tr className="bg-[#030E21]/50 border-b border-white/5 text-white/40 uppercase tracking-wider text-[9px] font-semibold">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {bills.map((bill) => (
                        <tr key={bill.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="p-3 font-mono text-white/50">{bill.dueDate}</td>
                          <td className="p-3 font-mono text-white/50">{bill.paymentDate || '—'}</td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[8px] font-semibold ${getStatusBadgeClass(bill.paymentStatus)}`}>
                          ...
                          <td className="p-3 text-center">
                            <button
                              ...
                              className="px-2.5 py-1 rounded bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 hover:text-white transition-all inline-flex items-center gap-1.5 text-[9px] font-semibold"
                            >
